...
## Process
1. **Survey** — read `readme.md` (lowercase), the `data/` directory tree, `docs/architecture-plan.md`, and recent history: `gh issue list --state all --limit 30 --json number,title,labels,state`. Understand what exists, what's in flight, and what recently shipped.
...
- Do not propose architectural refactors — those belong in `docs/architectural-suggestions.md`, not here.
