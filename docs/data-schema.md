...
- Bounties: `docs/schemas/bounty.v1.json` (`data/world-state/bounties.json`; the single world-scoped ledger of open player-funded mob bounties, parallel to `data/world-state/discovered-exits.json`). Each entry is one backer's `reward` gold stake on a mob type (`mob_template_id`, with `mob_name` captured at post time for listings/announcements) posted at `posted_tick`; `BOUNTY POST` appends an entry (at most one per `(backer, mob_template_id)` pair), `BOUNTY CANCEL` removes and refunds the backer's own entry, and a kill of a bountied mob type removes every matching entry and pays the pooled reward to the killer. The set is held in memory and mirrored to disk write-behind, so the tick-thread payout check never reads from disk; the loader is defensive (missing/empty/malformed file → no bounties) and the file is additive — its absence just means no bounties are open. The loader requires an exact `schema_version` match of `1`.
