...
| Economy (shops, auction, bank, guild treasury, mail gold) | ✅ | newbie bootstrap ✅ #519; auction search/sort QoL ✅ #565 · personal vault upgrades: `VAULT UPGRADE` at a bank NPC pays escalating gold (5,000 / 15,000 / 40,000) to permanently raise the player's own vault cap in +10-slot tiers `30→40→50→60` (per-player persisted `vaultTier`, defaults to 0 for existing saves, never regresses); `VAULT` shows slots used vs. effective capacity and the next tier's cost/gain ✅ #647 |
